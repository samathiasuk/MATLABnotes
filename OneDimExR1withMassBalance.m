...
MYsol=ode15s(@MYodefun,[t1(1) t1(end)],hI_vec,options,x,xB,S,T,h0);
%Evaluate solution at t1
hFD1=deval(MYsol,t1);

%Evaluate analytical solution
h=AnalyticalSol(t1,x,S,T,hI,h0);

%Evaluate solution at t2
hFD2=deval(MYsol,t2);
%Post processing with a finer resolution of times
for n=1:numel(t2)
    [dhdt,Mw(n),MwIN(n),MwOUT(n)]=MYodefun(t2(n),hFD2(:,n),x,xB,S,T,h0);
end
...
subplot(2,1,2)
hold on
plot(t2,(Mw-Mw(1))/1000,'o-b')
plot(t2,cumtrapz(t2,MwIN-MwOUT)/1000,'x-r')
ylabel('Metric tonnes of water that entered the aquifer')
xlabel('Time (days)')
...
function [dhdt,Mw,MwIN,MwOUT]=MYodefun(t,h,x,xB,S,T,h0)
...
%Determine mass of water in the system
rhoW=1000;
thetaW=S*h;
Mw=sum(rhoW*thetaW.*diff(xB,1),1);
MwIN=rhoW*Q(1);
MwOUT=rhoW*Q(end);
